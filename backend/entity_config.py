...
class EntityConfigLoader:
    ...
    def _load_config(self) -> None:
        ...
        try:
            if not os.path.exists(self.config_path):
                logger.warning(f"Entity config file not found at {self.config_path}, using defaults")
                self._load_defaults()
                return
            ...
            logger.info(f"Loaded entity config: {len(self.teams)} teams, {len(self.technologies)} technologies")
    ...
    def _load_defaults(self) -> None:
        ...
        self.teams = [
            TeamConfig("Frontend Team", ["frontend", "ui"], ["Frontend Developer"], "UI development"),
            TeamConfig("Backend Team", ["backend", "api"], ["Backend Developer"], "Server development"),
            TeamConfig("DevOps Team", ["devops", "ops"], ["DevOps Engineer"], "Infrastructure"),
        ]
    ...
    def get_project_patterns(self) -> List[str]:
        ...
        for project in self.projects:
            all_terms = [project.name.lower()] + [alias.lower() for alias in project.aliases]
            escaped_terms = [re.escape(term) for term in all_terms]
            pattern = r'\b(' + '|'.join(escaped_terms) + r')\b'
            patterns.append(pattern)
